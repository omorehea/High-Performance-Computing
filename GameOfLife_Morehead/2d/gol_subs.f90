...
subroutine ring_comm_2d(myid, myid_buffer, numprocs)
!--------------------------------------------------------------------------------------
!
!Description: Establishes ring-like communication between processors
!
...
!              myid_buffer(2) : processor to the left 
!              myid_buffer(3) : processor to the right
!              myid_buffer(4) : processor above
!              myid_buffer(5) : processor below
!              myid_buffer(6) : total number of processors
!
!--------------------------------------------------------------------------------------

  implicit none

  integer :: myid, numprocs, myid_left, myid_right, myid_above, myid_below
  integer :: myid_buffer(:)
  ...
  !-- only have code for a 2 x 2 grid of procs right now --
  ...
  myid_buffer(2) = myid_left
  myid_buffer(3) = myid_right
  myid_buffer(4) = myid_above
  myid_buffer(5) = myid_below
  myid_buffer(6) = numprocs


end subroutine ring_comm_2d
...
subroutine periodic_boundaries_init_2d(grid, myid_buffer)
!------------------------------------------------------------------------------ 
!
!Description: Sets the periodic boundary conditions for all the processors
!             for standard COLUMN-WISE domain decomposition
...
!------------------------------------------------------------------------------ 
  ...
  !-- top-most column of current grid --
  call MPI_ISEND(grid(2,2:(width-1)), width-2, MPI_INTEGER, myid_buffer(4), &
       &         1234, MPI_COMM_WORLD, request, ierr)

  call MPI_RECV(grid(height, 2:(width-1)), width-2, MPI_INTEGER, myid_buffer(5), &
       &        tag, MPI_COMM_WORLD, stat, ierr)

  
  !-- bottom-most column of current grid --
  
  call MPI_ISEND(grid(height-1,2:(width-1)), width-2, MPI_INTEGER, myid_buffer(5), &
       &         1234, MPI_COMM_WORLD, request, ierr)

  call MPI_RECV(grid(1, 2:(width-1)), width-2, MPI_INTEGER, myid_buffer(4), &
       &        tag, MPI_COMM_WORLD, stat, ierr)



  !-- handle the corners of the grid now --

  !-- procs recieve their top right corner --

  call MPI_ISEND(grid(1,2), 1, MPI_INTEGER, myid_buffer(2), &
       &         1234, MPI_COMM_WORLD, request, ierr)

  call MPI_RECV(grid(1,width), 1, MPI_INTEGER, myid_buffer(3), &
       &        tag, MPI_COMM_WORLD, stat, ierr)


  !-- procs recieve their bottom right corner --

  call MPI_ISEND(grid(2,width), 1, MPI_INTEGER, myid_buffer(4), &
       &         1234, MPI_COMM_WORLD, request, ierr)

  call MPI_RECV(grid(height,width), 1, MPI_INTEGER, myid_buffer(5), &
       &        tag, MPI_COMM_WORLD, stat, ierr)


  !-- procs recieve their top left corner --

  call MPI_ISEND(grid(1, width-1), 1, MPI_INTEGER, myid_buffer(2), &
       &         1234, MPI_COMM_WORLD, request, ierr)

  call MPI_RECV(grid(1,1), 1, MPI_INTEGER, myid_buffer(3), &
       &        tag, MPI_COMM_WORLD, stat, ierr)


  !-- procs recieve their bottom left corner --

  call MPI_ISEND(grid(2,1), 1, MPI_INTEGER, myid_buffer(4), &
       &         1234, MPI_COMM_WORLD, request, ierr)

  call MPI_RECV(grid(height,1), 1, MPI_INTEGER, myid_buffer(5), &
       &        tag, MPI_COMM_WORLD, stat, ierr)
...
end subroutine periodic_boundaries_init_2d
...
subroutine print_grid_total_2d(myid, numprocs, grid_local, dim_global, counts, displs, resizedsubarray, formatname)
!------------------------------------------------------------------------------  
!
!Description: Prints the current state of the grid to the screen
!             using ROW-WISE domain decomposition
...
!            resizedsubarray -- integer : derived data type for gatherv 2D grid decomposition
!            formatname -- character : used to print grid with less spacing between columns
!
!------------------------------------------------------------------------------  
  ...
  call MPI_GATHERV(grid_local_noghost, size(grid_local_noghost,1)*size(grid_local_noghost,2), MPI_INTEGER, &
       &           grid_total_nog, counts, displs, resizedsubarray, 0, MPI_COMM_WORLD, ierr)
...
end subroutine print_grid_total_2d
